module.exports = {
  status: "Development",
  name: "AC Control Version 1",
  description: "Controls version 1 AC units",
  ...
  actions: {
    set: {
      ...
      processData: function (data) {
        var values = require("./values.js").list.ACValuesV1;

        console.log("HELLO??");
        console.log(data);


        //First check if the unit is on or off
        if(data[1] == values.modes["off"][0] && data[5] == values.modes["off"][1]) {
          console.log("UNIT OFF");
        }
        else {
          //Find the mode and fan
          for(var i in values.modes) {
            for(var j in values.modes[i]) {
              var valid = true;
              if(valid && data[1] != values.modes[i][j][0]){valid=false;}
              if(valid && data[2] != values.modes[i][j][1]){valid=false;}
              if(valid && data[5] != values.modes[i][j][2]){valid=false;}
              if(valid && data[8] != values.modes[i][j][3]){valid=false;}
  
              console.log(data[1] + ":" + values.modes[i][j][0]);
              console.log(data[2] + ":" + values.modes[i][j][1]);
              console.log(data[5] + ":" + values.modes[i][j][2]);
              console.log(data[8] + ":" + values.modes[i][j][3]);
              console.log("" + i + j + valid);
              if(valid) {
                //We found the mode and fan!
                console.log("Mode = " + i);
                console.log("Fan = " + j);
              }
            }
